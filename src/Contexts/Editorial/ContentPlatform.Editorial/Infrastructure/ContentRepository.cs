...
internal sealed class ContentRepository(EditorialDbContext db) : IContentRepository
{
    ...
    public async Task<(IReadOnlyList<ContentItem> Items, int Total)> GetPagedAsync(Domain.EditorialStatus? status, string? search, int page, int size, bool ascending, CancellationToken ct)
    {
